...
export default function CustomerDetailPage({
  ...
  const statusStyles: Record<string, { label: string; className: string }> = {
    DRAFT: { label: tKarute('statusDraft'), className: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' },
    REVIEW: { label: tKarute('statusReview'), className: 'bg-yellow-50 text-yellow-700 dark:bg-yellow-950/30 dark:text-yellow-400' },
    APPROVED: { label: tKarute('statusApproved'), className: 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400' },
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div>
          <div className="flex items-start justify-between">
            ...
            <Button variant="outline" size="sm">{t('edit', { defaultValue: '編集' })}</Button>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <Card>
              <CardContent className="pt-4">
                ...
                <p className="text-xs text-muted-foreground">{t('visitCount')}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-4">
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                <p className="text-xs text-muted-foreground">{t('registeredDate')}</p>
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-muted-foreground">{t('aiInsights')}</h3>
              ...
              <CardContent className="py-3">
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium">
                    {t(`insightTypes.${insight.type}` as Parameters<typeof t>[0])}
                  </span>
                  <span className="text-[10px] text-muted-foreground">
                    {t('priority')} {Math.round(insight.priority * 100)}%
                  </span>
                </div>
      ...
      <Tabs defaultValue="timeline">
        <TabsList>
          ...
          <TabsTrigger value="karute">{t('tabs.karute')}</TabsTrigger>
